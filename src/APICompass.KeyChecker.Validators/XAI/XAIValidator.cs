...
namespace APICompass.KeyChecker.Validators.XAI;

public class XAIValidator : BaseKeyValidator
{
    private const string ApiUrl = "https://api.x.ai/v1";
    public override Provider Provider => Provider.XAI;

    public XAIValidator(HttpClient httpClient, IValidationCache cache, SemaphoreSlim semaphore)
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                {
                    IsValid = false,
                    ErrorMessage = "Invalid xAI key or blocked"
                };
            }
        ...
        catch
        {
            ...
            {
                IsValid = false,
                ErrorMessage = "xAI validation error"
            };
        }
    }
}
